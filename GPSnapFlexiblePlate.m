...
n2 = n+7;       % [-] amount of total training positions
...
Psi = blkdiag(Psi,Psi,Psi,Psi,[a;zeros(Nd,1)],[a;zeros(Nd,1)]); % 'MISO-MIMO structure'
...
hypZ = log([1e2 sqrt(1e-3) 1e2 sqrt(1e-3);
            1e0 sqrt(5e-2)  1e0 sqrt(5e-2)
            1e2 sqrt(1e-3) 1e2 sqrt(1e-3)
            5e0 sqrt(1e-5) 5e0 sqrt(1e-5)]);

for i = 1:4
    for ii = 1:npsiz
        hypGuess((i-1)*npsiz+ii).cov = hypZ(ii,:);
    end
end
hypGuess(npsi-1).cov = log([0.1667 2e2 sqrt(5e-2) sqrt(5e-2)]);
hypGuess(npsi).cov = log([sqrt(5e-2) 0.1667 2e2 sqrt(5e-2)]);
...
for i = n+1:n2
    ...
    hypOptSnap = minimize(hypOptSnap, @gp, -500, infMethod, meanfunc, covfunc{npsi-2,:}, likfunc, xTraining, Y);
    [mu, s2] = gp(hypOptSnap, infMethod, meanfunc, covfunc{npsi-2,:}, likfunc, xTraining, Y, xTest);
    figure(2); clf;
    subplot(121)
    ...
    [xstar,deltay] = OptimizeMI2D(xTest,xTraining,hypOptSnap,covfunc{npsi-2,:});
    subplot(122)
...
for i = 1:npsi
    ...
    subplot(plotTable(npsi,1),plotTable(npsi,2),i);
    imagesc(xpv,ypv,reshape(mu(:,i),grids,[]))
%     hold on
%     plot3(xTraining(:,1),xTraining(:,2),Y,'^','MarkerSize',15,'MarkerFaceColor',c2,'MarkerEdgeColor',c2);
%     xlabel('x [m]');ylabel('y [m]');zlabel('Snap Parameter [$kg/s^2$]');
end
...
for i =1:3
    ...
    ylabel('Scheduling Variable $y$ [$m$]');
    zlabel('$\|e\|_2$ [$m$]');
end
